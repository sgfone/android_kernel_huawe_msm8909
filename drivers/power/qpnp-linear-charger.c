...
#define LBC_START_AC_CHARGING            (0x30)
#define LBC_START_USB_CHARGING           (0x40)
#define LBC_STOP_CHARGING                (0x60)
...
/*Remove the unused code*/
    ...
    struct delayed_work     lbc_charger_work;
...
static int lbc_charge_work_flag = 1;
static struct qpnp_lbc_chip *lbc_device = NULL;
...
{
    struct qpnp_lbc_chip *chip = lbc_device;
...
static void charge_event_notify(unsigned int event)
{
    ...
    if(!chip)
    {
        pr_info("lbc device is not init, do nothing!\n");
        return;
    }
    /* avoid notify charge stop event continuously without charger inserted */
    if((chip->input_event != event) || (event == LBC_START_USB_CHARGING)
                                    || (event == LBC_START_AC_CHARGING))
    {
...
static void lbc_update_status(struct qpnp_lbc_chip *chip)
{
    unsigned int events = 0;
        if(!chip->batt_present)
        {
            events = LBC_STOP_CHARGING;
        }
    if(!events)
       {
            if(!chip->charger_disabled && chip->batt_present)
            {
                if(chip->event == CHARGER_TYPE_USB)
                {
                    events = LBC_START_USB_CHARGING;
                }
                if(chip->event == CHARGER_TYPE_STANDARD)
                {
                    events = LBC_START_AC_CHARGING;
                }
            }
       }
...
#define QPNP_LINEARCHARGER_TIMEOUT    (30000)

static void lbc_charger_work(struct work_struct *work)
{
    struct qpnp_lbc_chip *chip = container_of(work,
                                      struct qpnp_lbc_chip, lbc_charger_work.work);
    lbc_update_status(chip);
    schedule_delayed_work(&chip->lbc_charger_work,
                          msecs_to_jiffies(QPNP_LINEARCHARGER_TIMEOUT));
}

static void handle_charging_by_current_limit(struct qpnp_lbc_chip *chip)
{
    if (SUSPEND_CURRENT >= chip->usb_psy_ma)
    {
        pr_info("usb_phy_ma is less than 2mA\n");
    }
    /* check usb current_max current, whether it is 500mA */
    else if (SDP_CURRENT_LIMIT >= chip->usb_psy_ma)
    {
        chip->event = CHARGER_TYPE_USB;
        charge_event_notify(LBC_START_USB_CHARGING);
        schedule_delayed_work(&chip->lbc_charger_work, msecs_to_jiffies(0));
        lbc_charge_work_flag = 0;
    }
    /* check floated current, whether it is 1000mA */
    else if (IINLIM_1000 >= chip->usb_psy_ma)
    {
        chip->event = CHARGER_TYPE_STANDARD;
        charge_event_notify(LBC_START_AC_CHARGING);
        schedule_delayed_work(&chip->lbc_charger_work, msecs_to_jiffies(0));
        lbc_charge_work_flag = 0;
    }
    ...
    {
        pr_info(" else other mA chip->usb_psy_ma = %d \n",chip->usb_psy_ma);
        chip->event = CHARGER_TYPE_STANDARD;
        charge_event_notify(LBC_START_AC_CHARGING);
        schedule_delayed_work(&chip->lbc_charger_work, msecs_to_jiffies(0));
        lbc_charge_work_flag = 0;
...
/*Remove the unused code*/
	if (qpnp_lbc_is_usb_chg_plugged_in(chip)) {
        ...
        if((current_ma != chip->prev_max_ma) && lbc_charge_work_flag)
            {
                    mutex_lock(&chip->current_change_lock);
                    chip->usb_psy_ma = current_ma;
                    mutex_unlock(&chip->current_change_lock);
                    handle_charging_by_current_limit(chip);
            }
    }
    if(!qpnp_lbc_is_usb_chg_plugged_in(chip))
    {
        charge_event_notify(LBC_STOP_CHARGING);
        cancel_delayed_work_sync(&chip->lbc_charger_work);
        lbc_charge_work_flag = 1;
    }
...
static int qpnp_lbc_main_probe(struct spmi_device *spmi)
{
    ...
    INIT_DELAYED_WORK(&chip->lbc_charger_work, lbc_charger_work);
