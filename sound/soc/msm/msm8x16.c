...
{ 
	...
	{ 
        ...
        ad_logi("<%s> <%d>: Stop AFE_PORT_ID_QUATERNARY_MI2S_RX success.\n", __func__, __LINE__);
        ad_logi("%s:afe_port_stop_nowait tx\n",__func__);
        ...
        ad_logi("<%s> <%d>: Stop AFE_PORT_ID_QUATERNARY_MI2S_TX success.\n", __func__, __LINE__);
    } 
	return rc;
}
...
static int msm_external_pa_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    ...
    else
    {
        ...
        ad_logi("%s: afe_set_lpass_clock QUATERNARY_MI2S_RX\n", __func__);
        ret = afe_set_lpass_clock(AFE_PORT_ID_QUATERNARY_MI2S_RX, &lpass_mi2s_disable);
        if (ret < 0)  {
            ad_loge("%s: afe_set_lpass_clock failed QUATERNARY_MI2S_RX\n", __func__);
            //return ret;
        }     
        ad_logi("%s: afe_set_lpass_clock QUATERNARY_MI2S_TX\n", __func__);

        ret = afe_set_lpass_clock(AFE_PORT_ID_QUATERNARY_MI2S_TX, &lpass_mi2s_disable);
        if (ret < 0) 
        {
            ad_loge("%s: afe_set_lpass_clock failed QUATERNARY_MI2S_TX\n", __func__);
            //return ret;
        }
	}
	return ret;
...
static void msm_quat_mi2s_snd_shutdown(struct snd_pcm_substream *substream)
{
	...
	if ((!pdata->codec_type) &&
			((pdata->ext_pa & QUAT_MI2S_ID) == QUAT_MI2S_ID)) 
    {
        //if quat i2s port is opened by us, we must close it ourself in msm_external_pa_put, otherwise, close the port here
        if(0 == msm_quat_mi2s_clk)
        {
            ad_logd("%s:0 == msm_quat_mi2s_clk\n", __func__);
            ret = quat_mi2s_sclk_ctl(substream, false);
            if (ret < 0)
                ad_loge("%s:clock disable failed\n", __func__);
        }
